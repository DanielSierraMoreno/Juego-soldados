...
public class SuicideEnemy : EnemyTroops
{
	...
	private IEnumerator DoAttack(EnemyAttackPoints target = null)
	{
		...
		this.currentTarget.GetComponent<EnemyAttackPoints>().assignedTroops.Remove(this);

		int i = 0;
		while (this.GetComponent<AttackPointIsEnemy>().assignedTroops.Count > 0)
		{
			this.GetComponent<AttackPointIsEnemy>().assignedTroops[0].GetComponent<EnemyAttackPoints>().assignedTroops.Remove(this);
			this.GetComponent<AttackPointIsEnemy>().assignedTroops[0].ClearTarget();
			i++;
